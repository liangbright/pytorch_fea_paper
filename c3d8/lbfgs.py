...
class LBFGS(Optimizer):
    ...
    def __init__(self,
                 ...
                 history_size=100,
                 line_search_fn=None,
                 backtracking={"c":0.0001, "t_list": [1, 0.5, 0.1, 0.01, 0.001], "verbose":False, "flag":True}):
        if max_eval is None:
    ...
    def set_lr(self, lr=None, lr_init=None):
        group = self.param_groups[0]
        if lr is not None:
            group['lr']=lr
        if lr_init is not None:
            group['lr_init']=lr_init

    def backtracking(self, closure, loss0, gd, d, t_list, c, verbose):
        ...
        #-------------------
        t_best=t_list[0]
        self._add_grad(t_best, d)
        loss_best = float(closure())
        self._set_param(p_init)
        if loss_best < loss0+c*t_best*gd:
            t_list=[]
        #-------------------
        if len(t_list)>0:
            t_best=t_list[-1]
        ...
        if verbose == True:
            if loss_best > loss0:
                ...
                print("backtracking (good): t_best", t_best, "loss_best", loss_best, '< loss0', loss0)
        flag=loss_best < loss0
        return t_best, loss_best, flag

    def set_backtracking(self, t_list=None, c=None, verbose=None):
    ...
    def get_backtracking_flag(self):
        backtracking = self.param_groups[0]['backtracking']
        return backtracking['flag']

    def get_line_search_fn(self):
        group = self.param_groups[0]
    ...
    def step(self, closure):
        ...
        if np.isnan(loss):
            print("warning: loss is nan, abort")
            return False
        current_evals = 1
        state['func_evals'] += 1
        ...
        while n_iter < max_iter:
            ...
            if state['n_iter'] == 1:
                t = min(1., 1. / flat_grad.abs().sum()) * lr_init
            else:
                t = lr
            ...
            if gtd > -tolerance_change:
                print("gtd > -tolerance_change, break")#, break the current iteration")
                opt_cond=True
                break
            ...
            if line_search_fn is not None:
                ...
                if line_search_fn == "strong_wolfe":
                    ...
                    if state['n_iter'] == 1:
                        temp=float(min(1., 1. / flat_grad.abs().sum()))*lr_init
                        if t_list[-1]>temp:
                            t_list=t_list.copy()
                            t_list.append(temp)
                    else:
                        t_list=backtracking["t_list"]
                    c_back=backtracking["c"]
                    verbose=backtracking['verbose']
                    t, loss, flag = self.backtracking(closure, loss, gtd, d, t_list, c_back, verbose)
                    #---------------------
                    backtracking['flag']=flag
                    self._add_grad(t, d)
                    opt_cond = flat_grad.abs().max() <= tolerance_grad
                else:

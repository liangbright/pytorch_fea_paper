...
must_points=''
#%% only for test
shell_template_str='./app1/bav17_AortaModel_P0_best'
mesh_p0_str='./app1/p0_171_solid'
mesh_px_str='./app1/p0_171_solid_matMean_p20'
mat_str=matMean
px_pressure=20
#%%
import argparse
...
parser.add_argument('--max_iter2', default=100, type=int)
...
if arg.mat_model == "GOH_SRI":
    ...
    from Mat_GOH_Jv import cal_1pk_stress, cal_cauchy_stress
    Mat=[float(m) for m in arg.mat.split(",")]
    Mat[4]=np.pi*(Mat[4]/180)
    Mat=torch.tensor([Mat], dtype=dtype, device=device)
...
optimizer.set_backtracking(t_list=[0.5, 0.3, 0.1, 0.05, 0.01], t_default=0.5, t_default_init=0.5)
...
while iter1 <= max_iter1:
    ...
    while iter2 <= max_iter2:
        ...
        if iter2==1 or opt_cond is True:
            print('iter2', iter2, 'Rmean', Rmean, 'Rmax', Rmax, "force_avg", float(force_avg))

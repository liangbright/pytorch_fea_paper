# -*- coding: utf-8 -*-
"""
Created on Sun Jun  6 19:14:26 2021
...
from FEModel_C3D8_fiber import cal_F_tensor_8i, cal_F_tensor_1i
from FEModel_C3D8_fiber import cal_nodal_force_from_1pk_stress_8i, cal_nodal_force_from_1pk_stress_1i
from FEModel_C3D8_fiber import cal_d_sf_dX_and_dX_dr_8i, cal_d_sf_dX_and_dX_dr_1i
from FEModel_C3D8_fiber import cal_strain_energy, cal_potential_energy, cal_1pk_stress_force
from FEModel_C3D8_fiber import cal_pressure_force_quad_1i as cal_pressure_force
#%%
def cal_element_orientation(node, element):
    ...
    x7=node[element[:,6]]
    x8=node[element[:,7]]
    a=(x2+x3+x6+x7)-(x1+x4+x5+x8)
    d=(x3+x4+x7+x8)-(x1+x2+x5+x6)
...
class AortaFEModel:
    def __init__(self, node_x, element, node_X, boundary0, boundary1, inner_surface,
                 material, cal_1pk_stress, cal_cauchy_stress, dtype, device, mode, reduced_integration=False):
        ...
        #reduced_integration: True then C3D8R
        #This model also works for Fbar and 3Fields
    ...
    def initialize_for_inflation(self):
        ...
        if self.reduced_integration == False:
            d_sf_dX, dX_dr, det_dX_dr=cal_d_sf_dX_and_dX_dr_8i(state['node_X'], state['element'])
        else:
            d_sf_dX, dX_dr, det_dX_dr=cal_d_sf_dX_and_dX_dr_1i(state['node_X'], state['element'])
    ...
    def cal_energy_and_force_for_inflation(self, pressure, return_stiffness=None):
        ...
        if return_stiffness is None:
            force_int, F, S, W, force_int_of_element=Output_int
        else:
            force_int, F, S, W, H_int, force_int_of_element=Output_int
            H_int=process_H(H_int, state['free_node'])
            H=H_int-H_ext
        SE=cal_strain_energy(W, state['det_dX_dr'], reduction="sum")
    ...
    def cal_energy_and_force_for_inverse_p0(self, pressure, return_stiffness=None, detach_X=True):
        ...
        if return_stiffness is None:
            ...
            if isinstance(H_ext, torch.Tensor) == True:
                H_ext=process_H(H_ext, state['free_node'])
        if self.reduced_integration == False:
            d_sf_dX, dX_dr, det_dX_dr=cal_d_sf_dX_and_dX_dr_8i(node_X, state['element'])
        else:
            d_sf_dX, dX_dr, det_dX_dr=cal_d_sf_dX_and_dX_dr_1i(node_X, state['element'])
        if self.element_orientation_on_deformed_mesh == False:
            element_orientation=cal_element_orientation(node_X, state['element'])
        else:
            element_orientation=cal_element_orientation(node_x, state['element'])
        ...
        if return_stiffness is None:
            ...
            H_int=process_H(H_int, state['free_node'])
            H=H_int-H_ext
        SE=cal_strain_energy(W, det_dX_dr, det_dX_dr, reduction="sum")
        force_ext1=force_ext*state['mask']
        force_ext2=force_int*(1-state['mask'])
        force_ext=force_ext1+force_ext2
        TPE1=SE-cal_potential_energy(force_ext1, u_field)
    ...
    def cal_energy_and_force_for_inverse_mat(self, pressure):
        ...
        if self.element_orientation_on_deformed_mesh == False:
            ...
            element_orientation=cal_element_orientation(state['node_x'], state['element'])
            element_orientation=element_orientation.detach()
        force_int, F, S, W, force_int_of_element=cal_1pk_stress_force(state['node_x'],
                                                                      state['element'],
                                                                      state['d_sf_dX'],
                                                                      state['det_dX_dr'],
                                                                      state['material'],
    ...
    def cal_F_tensor(self):
        ...
        if self.mode == "inflation":
            ...
            node_x=node_X+u_field
            if self.reduced_integration == False:
                F=cal_F_tensor_8i(node_x, state['element'], node_X, d_sf_dX=state['d_sf_dX'])
            else:
                F=cal_F_tensor_1i(node_x, state['element'], node_X, d_sf_dX=state['d_sf_dX'])
        elif self.mode == "inverse_p0":
            node_x=state['node_x']
            u_field=self.get_u_field()
            node_X=node_x-u_field
            if self.reduced_integration == False:
                F=cal_F_tensor_8i(node_x, state['element'], node_X)
            else:
                F=cal_F_tensor_1i(node_x, state['element'], node_X)
        elif self.mode == "inverse_mat":
            node_x=state['node_x']
            node_X=state['node_X']
            if self.reduced_integration == False:
    ...
    def cal_stress(self, stress, create_graph=False, return_W=False, local_sys=False):
        ...
        if stress == 'cauchy':
            S, W=self.cal_cauchy_stress(F, state['material'], orientation, create_graph=create_graph, return_W=True)
        elif stress == '1pk':
    ...
    def cal_pre_stress(self, stress, create_graph=False, return_W=False, local_sys=False):
        state=self.state
        if self.mode == "inflation":
            node_X=state['node_X']
            u_field=self.get_u_field()
